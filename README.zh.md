...
规则的匹配维度是 **工具 + 请求的沙箱模式 + 命令开头几个词**，所以允许了 `pnpm dsh plugin` 不会顺带允许 `rm`；为 `danger-full-access` 记的规则也不会覆盖别的请求。前缀的推导：去掉开头 `cd … &&`、去掉 `VAR=value`、程序名取 basename（`/opt/homebrew/bin/gh` → `gh`），然后一直取到第一个选项、路径或 shell 操作符。**按钮上写的就是将要记住的那串前缀**。
